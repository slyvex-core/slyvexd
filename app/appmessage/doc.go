/*
Package appmessage implements the slyvex appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported slyvex messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# slyvex Message Overview

The slyvex protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which slyvex network it is a part of, its type, how big it is, and a checksum
...
of the currently supported slyvex messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using slyvex encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the slyvex messages are intended
...
and write slyvex messages. The following sections provide a quick overview of
...
The slyvex network is a magic number which is used to identify the start of a
message and which slyvex network the message applies to. This package provides
...
As discussed in the slyvex message overview section, this package reads
and writes slyvex messages using a generic interface named Message. In
...
In order to unmarshall slyvex messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a slyvex peer. Example syntax is:

	// Reads and validates the next slyvex message from conn using the
	// protocol version pver and the slyvex network slyvexNet. The returns
	// are a appmessage.Message, a []byte which contains the unmarshalled
	// raw payload, and a possible error.
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, slyvexNet)
	if err != nil {
...
In order to marshall slyvex messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a slyvex peer. Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the slyvex network slyvexNet. The return is a possible
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, slyvexNet)
	if err != nil {
		// Log and handle the error
	}
