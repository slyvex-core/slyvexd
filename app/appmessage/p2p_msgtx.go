...
// Outpoint defines a slyvex data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new slyvex transaction outpoint point with the
...
// TxIn defines a slyvex transaction input.
type TxIn struct {
...
// NewTxIn returns a new slyvex transaction input with the provided
...
// TxOut defines a slyvex transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new slyvex transaction output with the provided
...
// MsgTx implements the Message interface and represents a slyvex tx message.
