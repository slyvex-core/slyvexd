...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("slyvexdsanity-slyvexd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("slyvexd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "SLYVEXD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "SLYVEXD-STDERR")
