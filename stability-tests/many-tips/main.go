...
import (
	...
	"github.com/slyvex-core/slyvexd/domain/consensus/utils/mining"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("slyvexd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("slyvexd datadir: %s", dataDir)

	SlyvexdCmd, err := common.StartCmd("SLYVEXD",
		"slyvexd",
	...
	spawn("startNode-SlyvexdCmd.Wait", func() {
		err := SlyvexdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("SlyvexdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes slyvexd closes ungracefully
				log.Errorf("SlyvexdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(SlyvexdCmd, "SlyvexdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	slyvexMinerCmd, err := common.StartCmd("MINER",
		"slyvexminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("slyvex-miner-Cmd.Wait", func() {
		err := slyvexMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(slyvexMinerCmd, "slyvexMinerCmd")
	return nil
}
