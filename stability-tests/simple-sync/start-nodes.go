...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("slyvexd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("slyvexd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("SLYVEXD-SYNCER",
		"slyvexd",
	...
	syncedCmd, err := common.StartCmd("SLYVEXD-SYNCED",
		"slyvexd",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
