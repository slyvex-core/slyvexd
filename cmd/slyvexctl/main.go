...
import (
	...
	"github.com/slyvex-core/slyvexd/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		slyvexdMessage, err := client.Post(&protowire.SlyvexdMessage{Payload: &protowire.SlyvexdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := slyvexdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	slyvexdMessage := &protowire.SlyvexdMessage{}
	err := protojson.Unmarshal([]byte(response), slyvexdMessage)
	if err != nil {
	...
	return marshalOptions.Format(slyvexdMessage)
}
