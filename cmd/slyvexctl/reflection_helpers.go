...
// 2. A wrapper that implements isSlyvexdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `SlyvexdMessage_` prefix and `Request` suffix
...
// generateSlyvexdMessage generates a wrapped SlyvexdMessage with the given `commandValue`
func generateSlyvexdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.SlyvexdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	slyvexdMessage := reflect.New(reflect.TypeOf(protowire.SlyvexdMessage{}))
	slyvexdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return slyvexdMessage.Interface().(*protowire.SlyvexdMessage), nil
}
