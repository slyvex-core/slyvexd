...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libslyvexwallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libslyvexwallet.SlyvexwalletdUTXOsTolibslyvexwalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatSvx(splitTransactions[i].Outputs[0].Value), " SVX")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatSvx(totalExtracted), " SVX")
