...
import (
	...
	"github.com/slyvex-core/slyvexd/domain/consensus/utils/testutils"
...
func TestMultisig(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				var err error
				mnemonics[i], err = libslyvexwallet.CreateMnemonic()
				if err != nil {
					t.Fatalf("CreateMnemonic: %+v", err)
				}

				publicKeys[i], err = libslyvexwallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], true)
				if err != nil {
			...
			path := "m/1/2/3"
			address, err := libslyvexwallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
			if err != nil {
			...
			selectedUTXOs := []*libslyvexwallet.UTXO{
				{
			...
			unsignedTransaction, err := libslyvexwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
				[]*libslyvexwallet.Payment{{
					Address: address,
					Amount:  10,
				}}, selectedUTXOs)
			if err != nil {
			...
			isFullySigned, err := libslyvexwallet.IsTransactionFullySigned(unsignedTransaction)
			if err != nil {
			...
			_, err = libslyvexwallet.ExtractTransaction(unsignedTransaction, ecdsa)
			...
			signedTxStep1, err := libslyvexwallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			isFullySigned, err = libslyvexwallet.IsTransactionFullySigned(signedTxStep1)
			if err != nil {
				t.Fatalf("IsTransactionFullySigned: %+v", err)
			}
			...
			signedTxStep2, err := libslyvexwallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxStep2, err := libslyvexwallet.ExtractTransaction(signedTxStep2, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}

			signedTxOneStep, err := libslyvexwallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxOneStep, err := libslyvexwallet.ExtractTransaction(signedTxOneStep, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libslyvexwallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			signedTx, err := libslyvexwallet.Sign(params, mnemonics, unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			tx, err := libslyvexwallet.ExtractTransaction(signedTx, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
...
func TestMaxSeep(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		params := &consensusConfig.Params
		cfg := *consensusConfig
		cfg.BlockCoinbaseMaturity = 0
		cfg.PreDeflationaryPhaseBaseSubsidy = 20e6 * constants.SompiPerSlyvex
		...
		for i := 0; i < numKeys; i++ {
			...
			publicKeys[i], err = libslyvexwallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false)
			if err != nil {
				t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
			}
		}
		...
		const minimumSignatures = 1
		path := "m/1/2/3"
		address, err := libslyvexwallet.Address(params, publicKeys, minimumSignatures, path, false)
		if err != nil {
			t.Fatalf("Address: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAmount := []*libslyvexwallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAmount, err := libslyvexwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libslyvexwallet.Payment{{
				Address: address,
				Amount:  10,
		...
		signedTxWithLargeInputAmount, err := libslyvexwallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAmount, err := libslyvexwallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libslyvexwallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAndOutputAmount, err := libslyvexwallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*libslyvexwallet.Payment{{
				Address: address,
				Amount:  22e6 * constants.SompiPerSlyvex,
			}}, selectedUTXOsForTxWithLargeInputAndOutputAmount)
		if err != nil {
			t.Fatalf("CreateUnsignedTransactions: %+v", err)
		}

		signedTxWithLargeInputAndOutputAmount, err := libslyvexwallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAndOutputAmount, err := libslyvexwallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
