...
// Takes in a string representation of the Svx value to convert to Seep
func TestSvxToSeep(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := SvxToSeep(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := SvxToSeep(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateSVXAmountFormat(testCase)
