...
// FormatSvx takes the amount of seeps as uint64, and returns amount of SVX with 8  decimal places
func FormatSvx(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.SompiPerSlyvex)
	}
	return res
}

// SvxToSeep takes in a string representation of the Svx value to convert to Seep
func SvxToSeep(amount string) (uint64, error) {
	err := validateSVXAmountFormat(amount)
	...
	if constants.SompiPerSlyvex%10 != 0 {
		return 0, errors.Errorf("Unable to convert to seep when SompiPerSlyvex is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerSlyvex))
	decimalStr := ""
...
func validateSVXAmountFormat(amount string) error {
