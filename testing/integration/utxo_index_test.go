...
func TestUTXOIndex(t *testing.T) {
	// Setup a single slyvexd instance
	...
	err := slyvexd.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := slyvexd.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSeep := uint64(blockAmountToMine * constants.SompiPerSlyvex * 500)
	getBlockCountResponse, err := slyvexd.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountSeep := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SompiPerSlyvex * 500, // -2 because of genesis and virtual.
	)
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = slyvexd.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := slyvexd.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) *appmessage.RPCTransaction {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixSlyvexSim)
	if err != nil {
