...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of slyvexd instances
	slyvexd1, slyvexd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := slyvexd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In slyvexd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, slyvexd2)
	}

	// Connect the two slyvexds. This should trigger sync
	// between the two nodes
	connect(t, slyvexd1, slyvexd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := slyvexd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
