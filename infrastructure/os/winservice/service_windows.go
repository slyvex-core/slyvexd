...
// long-running slyvexdMain (which is the real meat of slyvexd), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start slyvexdMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once slyvexd is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("slyvexdMain-windows", func() {
...
// logServiceStart logs information about slyvexd when the main server has
