...
const (
	defaultConfigFilename      = "slyvexd.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "slyvexd.log"
	defaultErrLogFilename      = "slyvexd_err.log"
	...
	sampleConfigFilename    = "sample-slyvexd.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for slyvexd.
	DefaultAppDir = util.AppDir("slyvexd", false)
...
// Flags defines the configuration options for slyvexd.
//
...
type Flags struct {
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 22110, testnet: 22210)"`
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in SVX/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for slyvexd.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default slyvexd configuration
...
//
// The above results in slyvexd functioning properly without any config settings
...
// createDefaultConfig copies the file sample-slyvexd.conf to the given destination path,
