...
type GetBlockTemplateRequestMessage struct {
	...
	// Which slyvex address should the coinbase block reward transaction pay into
...
type GetBlockTemplateResponseMessage struct {
	...
	// Whether slyvexd thinks that it's synced.
	// Callers are discouraged (but not forbidden) from solving blocks when slyvexd is not synced.
	// That is because when slyvexd isn't in sync with the rest of the network there's a high
...
type GetConnectedPeerInfoMessage struct {
	...
	// Whether this slyvexd initiated the connection
	...
	// The timestamp of when this peer connected to this slyvexd
...
type GetCoinSupplyResponseMessage struct {
	...
	MaxSeep         uint64    `protobuf:"varint,1,opt,name=maxSeep,proto3" json:"maxSeep,omitempty"` // note: this is a hard coded maxSupply, actual maxSupply is expected to deviate by upto -5%, but cannot be measured exactly.
	CirculatingSeep uint64    `protobuf:"varint,2,opt,name=circulatingSeep,proto3" json:"circulatingSeep,omitempty"`
	Error            *RPCError `protobuf:"bytes,1000,opt,name=error,proto3" json:"error,omitempty"`
}
...
func (x *GetCoinSupplyResponseMessage) GetMaxSeep() uint64 {
	if x != nil {
		return x.MaxSeep
	}
	return 0
...
func (x *GetCoinSupplyResponseMessage) GetCirculatingSeep() uint64 {
	if x != nil {
		return x.CirculatingSeep
	}
	return 0
}
