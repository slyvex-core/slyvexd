...
Clients are expected to build RequestMessages and wrap them in SlyvexdMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a SlyvexdMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a SlyvexdMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network slyvexd is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which slyvex address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether slyvexd thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when slyvexd is not synced. That is because when slyvexd isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known slyvexd addresses in the
...
currently connected to this slyvexd.
...
| isOutbound | [bool](#bool) |  | Whether this slyvexd initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this slyvexd |
...
AddPeerRequestMessage adds a peer to slyvexd&#39;s outgoing connection list.
This will, in most cases, result in slyvexd connecting to said peer.
...
parent chain from some startHash to this slyvexd&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this slyvexd.
...
ShutDownRequestMessage shuts down this slyvexd.
...
This call is only available when this slyvexd was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given slyvexd addresses
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| maxSeep | [uint64](#uint64) |  | note: this is a hard coded maxSupply, actual maxSupply is expected to deviate by upto -5%, but cannot be measured exactly. |
| circulatingSeep | [uint64](#uint64) |  |  |
| error | [RPCError](#protowire.RPCError) |  |  |
