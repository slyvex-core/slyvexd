...
func (c *coinbaseManager) ExpectedCoinbaseTransaction(stagingArea *model.StagingArea, blockHash *externalapi.DomainHash,
	...
	for _, blue := range ghostdagData.MergeSetBlues() {
		txOut, devTx, hasReward, err := c.coinbaseOutputForBlueBlock(stagingArea, blue, acceptanceDataMap[*blue], daaAddedBlocksSet)
		if err != nil {
			return nil, false, err
		}
	...
	txOut, devTx, hasRedReward, err := c.coinbaseOutputForRewardFromRedBlocks(
...
func (c *coinbaseManager) coinbaseOutputForBlueBlock(stagingArea *model.StagingArea,
	...
	devFeeDecodedAddress, err := util.DecodeAddress(devFeeAddress, util.Bech32PrefixSlyvex)
	if err != nil {
		return nil, nil, false, err
	}
